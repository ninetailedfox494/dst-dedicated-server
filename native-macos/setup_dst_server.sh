...
phase_1_prerequisites() {
    ...
    if [[ ! -f "${PROJECT_ROOT}/env/worldgenoverride.txt" ]]; then
        warn "env/worldgenoverride.txt not found"
        if [[ -f "${PROJECT_ROOT}/env/worldgenoverride.txt.template" ]]; then
            cp "${PROJECT_ROOT}/env/worldgenoverride.txt.template" "${PROJECT_ROOT}/env/worldgenoverride.txt"
            log "Created env/worldgenoverride.txt from template"
        fi
    fi
...
EOF
    ...
    # Generate worldgenoverride.lua for Master and Caves
    WORLDGEN_TEMPLATE="${PROJECT_ROOT}/env/worldgenoverride.txt"
    if [[ ! -f "${WORLDGEN_TEMPLATE}" ]]; then
        err "env/worldgenoverride.txt not found. Copy env/worldgenoverride.txt.template first."
    fi
    cp "${WORLDGEN_TEMPLATE}" "${MASTER_DIR}/worldgenoverride.lua"
    WORLDGEN_SIZE="${DST_WORLD_SIZE:-medium}"
    sed "s/__DST_WORLD_SIZE__/${WORLDGEN_SIZE}/g" "${MASTER_DIR}/worldgenoverride.lua" > "${MASTER_DIR}/worldgenoverride.lua.tmp"
    mv "${MASTER_DIR}/worldgenoverride.lua.tmp" "${MASTER_DIR}/worldgenoverride.lua"
